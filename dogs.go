...
func NewDog(name string, breed string, age int) *Dog {
	dog := Dog{}
	dog.Pet = *NewPet(name, "Dog", breed, age)
	return &dog
}

func RandomDog() *Dog {
	name := Names[rand.Intn(len(Names))]
	breed := DogBreeds[rand.Intn(len(DogBreeds))]
	age := rand.Intn(23) + 1
	return NewDog(name, breed, age)
}

var DogBreeds = []string{"Affenpinscher", "Afghan Hound", "Airedale Terrier", "Akita",
